...
export class TransactionList extends React.Component {
    ...
    this.props.transactions.map(transaction => {
      if (transaction.id === id) {
        if (transaction.type === "Transfer") {
          return this.props.startDeleteTransfer(idObject, transaction);
        } else {
          return this.props.startDeleteTransaction(idObject, transaction);
        }
      } else {
        return null;
      }
    ...
    this.props.transactions.map(transaction => {
      if (transaction.id === id) {
        if (transaction.type === "Transfer") {
          ...
          return this.props.startAddTransaction(transaction);
        }
      } else {
        return null;
      }
  ...
  onFilter = ({
    ...
    this.props.setAccountFilter(accountFilter);
    this.props.setCategoryFilter(categoryFilter);
  };
        ...
        <Table className={classes.table}>
          ...
          <TableBody>
            ...
            this.props.transactions.hasOwnProperty(0) === false ? (
              ...
              _.orderBy(this.props.transactions, ["date"], ["desc"])
                .slice(
          ...
          <TableFooter>
            <TableRow>
              <TablePagination
                count={
                  this.props.transactions.length !== undefined
...
const mapDispatchToProps = (dispatch, props) => ({
  startDeleteTransfer: (id, transaction) =>
    dispatch(startDeleteTransaction(id))
      .then(() => {
        dispatch(
          updateAccountBalance(transaction.transferFrom, transaction.amount)
        );
      })
      .then(() => {
        dispatch(
          updateAccountBalance(transaction.transferTo, -transaction.amount)
        );
      }),
  ...
  startDeleteTransaction: (id, transaction) =>
  ...
  setDescriptionFilter: descriptionFilter =>
    dispatch(setDescriptionFilter(descriptionFilter)),
